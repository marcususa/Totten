...
class MixedAnalysis(ctk.CTkFrame, CatalogInitMixin, MixedAnalysisNavigation, MixedAnalysisEngine):
    ...
    def __init__(self, parent, filename=None, initial_games=None, active_focus=None, active_index=None, *args,
                 **kwargs):
        ...
        # Prevent the player panel treeview from capturing arrow keys so the board keeps exclusive control
        if hasattr(self, "pgn_tree") and self.pgn_tree:
            try:
                self.pgn_tree.unbind("<Up>")
                self.pgn_tree.unbind("<Down>")
            except Exception:
                pass

        if hasattr(self, "moves_textbox") and self.moves_textbox:
            try:
                self.moves_textbox.configure(wrap="word")
            except Exception:
                pass
        ...
        # Bind navigation and flip keys to both top-level and self
        top = self.winfo_toplevel()
        for target in (top, self):
            target.bind("<Left>", self.on_prev_move)
            target.bind("<Right>", self.on_next_move)
            target.bind("<Up>", self.on_first_move)
        ...
        if hasattr(self, "board_widget") and self.board_widget:
            ...
            # Instantly grab focus back to the workspace when clicking the board
            self.board_widget.bind("<Button-1>", lambda e: self.focus_set())
            for child in self.board_widget.winfo_children():
